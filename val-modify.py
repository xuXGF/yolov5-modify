...
def run(data,
    ...
    for batch_i, (img, targets, paths, shapes) in enumerate(tqdm(dataloader, desc=s)):
        ...
        #                             augment=augment,
        #                             visualize=increment_path(save_dir / Path(path).stem, mkdir=True) if visualize else False)[0]
        #             # pred_temp, train_out = model(img, augment=augment)
...
def parse_opt():
    ...
    parser.add_argument('--data', type=str, default=ROOT / 'data-bosch.yaml', help='dataset.yaml path')
    # parser.add_argument('--weights', nargs='+', type=str, default=ROOT / '../result/bosch+s/weights/best.pt', help='model.pt path(s)')
    parser.add_argument('--weights', nargs='+', type=str, default=ROOT / '../result-end/bosch+s+small+cbam/weights/best.pt', help='model.pt path(s)')
    parser.add_argument('--batch-size', type=int, default=2, help='batch size')
    parser.add_argument('--imgsz', '--img', '--img-size', type=int, default=960, help='inference size (pixels)')
    parser.add_argument('--mulpicplus', type=str, default="2",help='  1 for normal,2 for 4pic plus,3 for 9pic plus and so on')
